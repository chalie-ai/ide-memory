...
**MCP tools not available (Claude Code)**: Verify with `claude mcp list` — the `memory` server should appear. If not, run `claude mcp add --scope user --transport http memory http://localhost:8080/mcp`.
